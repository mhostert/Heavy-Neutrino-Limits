...
lepton='e'
operator_NB_e = fr'$\mathcal{{O}}_{{\rm{{NB}}}}^{{{lepton}}}: (\overline{{{{L}}_{{{lepton}}}}} \sigma_{{\mu\nu}} N) {{\widetilde H}} B^{{\mu\nu}}$'
lepton='\mu'
operator_NB_mu = fr'$\mathcal{{O}}_{{\rm{{NB}}}}^{{{lepton}}}: (\overline{{{{L}}_{{{lepton}}}}} \sigma_{{\mu\nu}} N) {{\widetilde H}} B^{{\mu\nu}}$'
lepton='\\tau'
operator_NB_tau = fr'$\mathcal{{O}}_{{\rm{{NB}}}}^{{{lepton}}}: (\overline{{{{L}}_{{{lepton}}}}} \sigma_{{\mu\nu}} N) {{\widetilde H}} B^{{\mu\nu}}$'

lepton='e'
operator_NW_e = fr'$\mathcal{{O}}_{{\rm{{NW}}}}^{{{lepton}}}: (\overline{{{{L}}_{{{lepton}}}}} \sigma_{{\mu\nu}} N) {{\tau^{{\alpha}}}} {{\widetilde H}} B^{{\mu\nu}}_{{\alpha}}$'
lepton='\mu'
operator_NW_mu = fr'$\mathcal{{O}}_{{\rm{{NW}}}}^{{{lepton}}}: (\overline{{{{L}}_{{{lepton}}}}} \sigma_{{\mu\nu}} N) {{\tau^{{\alpha}}}} {{\widetilde H}} B^{{\mu\nu}}_{{\alpha}}$'
lepton='\\tau'
operator_NW_tau = fr'$\mathcal{{O}}_{{\rm{{NW}}}}^{{{lepton}}}: (\overline{{{{L}}_{{{lepton}}}}} \sigma_{{\mu\nu}} N) {{\tau^{{\alpha}}}} {{\widetilde H}} B^{{\mu\nu}}_{{\alpha}}$'

o_dic = {'dipole': {'e': '', 'mu': '', 'tau': ''}, 
         'CNB': {'e': operator_NB_e, 'mu': operator_NB_mu, 'tau': operator_NB_tau},
         'CNW': {'e': operator_NW_e, 'mu': operator_NW_mu, 'tau': operator_NW_tau}}
...
def std_plot_limits(case,
        ...
        if (id not in skip_ids) & (limit.interp_func is not None):
            ...
            if limit.file_top == limit.file_bottom and limit.m4 is not None and limit.ualpha4 is not None:
                ...
                if (limit.year is None):
                    continue
                    #ax.plot(limit.m4, limit.ualpha4, color=contour_color, dashes=dash, zorder=3, lw=LW)
                    #ax.fill(limit.m4, limit.ualpha4, facecolor=fill_color, edgecolor='None', alpha=ALPHA, zorder=U_MEAN, hatch='///')
                else:
                    ax.fill(limit.m4, limit.ualpha4, facecolor=fill_color, edgecolor='None', alpha=ALPHA, zorder=U_MEAN)

            else:
                if ('bbn' in id) or ('lsnd' in id) or ('sk_atm' in id):
                    ax.plot(limit.m4, limit.ualpha4, color=contour_color, dashes=dash, zorder=1, lw=LW)
                else:
                    if ('sn' in id):
                        ax.plot(x, limit.interp_func_top(x), color=contour_color, dashes=dash, zorder=1, lw=LW)
                        ax.plot(limit.m4, limit.ualpha4, color=contour_color, dashes=dash, zorder=1, lw=LW)
    #                        ax.plot(limit.file_bottom.m4, limit.file_bottom.ualpha4, color=contour_color, dashes=dash, zorder=1, lw=LW)
                    else:
                        ax.plot(x, limit.interp_func(x), color=contour_color, dashes=dash, zorder=1, lw=LW)
                        ax.plot(x, limit.interp_func_top(x), color=contour_color, dashes=dash, zorder=1, lw=LW)
                
                # Filling
                if ('bbn' in  id):# or (limit.year is None):
                    ax.fill_between(limit.m4, limit.ualpha4, color=contour_color, edgecolor='None', alpha=0.1, zorder=U_MEAN)
                #if ('sk_atm' in  id):# or (limit.year is None):
                #    ax.fill_between(limit.m4, limit.ualpha4, color=contour_color, edgecolor='None', alpha=0.1, zorder=U_MEAN)
                if ('sn' ==  id):
                    #continue
                    ax.fill_between(limit.m4, limit.ualpha4, limit.interp_func_top(limit.m4) , color=contour_color, edgecolor='None', alpha=0.1, zorder=U_MEAN)#, hatch='/')
                # elif  id not in color_only and len(color_only) > 0:
    ...
    ax.legend(title=o_dic['%s'%case.operator]['%s'%case.flavor], loc='lower right', fontsize=8)
    ax.set_yscale("log")
    ax.set_xscale("log")
