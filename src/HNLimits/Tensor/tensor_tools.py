...
operator_coefficient_dict= {'dipole': 1, 'CNB': 6.492e-3, 'CNW': 1.233e-2} # Relations between d_gamma and CNB, or CNB assuming one operator at a time. CNB and CNW numbers in [1/GeV]. See eq. (4.14). 

dipole_units_dict = {'invGeV': 1, 'muB': 296.30/2} #Transforms Bohr magneton to 1/GeV. $\mu_B = e/2m_e \simeq 296.30 GeV^{-1}$. Also \mu_tran = 2d. We get dipole moment in 1/GeV units.

#cl90_dict = {'1sigma': sqrt(4.61/2.30), 90: 1, 95: sqrt(4.61/5.99), '2sigma': sqrt(4.61/6.18)} # 2dof chi^2 relations 
...
exp_dict = {'borexino', 'borexino_sk', 'lep', 'lsnd', 'sn', 'xenon'}
...
class Limits:
    def __init__(self, operator='dipole', flavor='e', invisible=False):
        """ Class that contains all limits on a HNL given a certain flavor structure

        Args:
            operator (str, optional): the tensor operator we want to plot. Can be 'dipole', 'CNB', or 'CNW'.Defaults to 'dipole'.
        ...
        if self.operator == 'dipole':
            self.latexflavor = fr'$d_{{{subscript}}} ~(\rm{{GeV}}^{{-1}})$'
        if self.operator == 'CNB':
            self.latexflavor = fr'$C_\text{{NB}}^{{{subscript}}}/\Lambda^2 ~(\rm{{GeV}}^{{-2}})$'
        if self.operator == 'CNW':
            self.latexflavor = fr'$C_\text{{NW}}^{{{subscript}}}/\Lambda^2 ~(\rm{{GeV}}^{{-2}})$'
        self.invisible = invisible
        _df = load_google_sheet(sheet_name=f'dipole_{flavor}')
        ...
        self.path_to_plot = os.path.join(os.getcwd(), f'plots/tensor/{self.operator}{self.flavor}_majorana.pdf')
    ...
    def get_data(self, df, top = False):
        ...
        if (limit_path is None):
            ...
            if (self.invisible and not df.is_invisible):
                ...
                # re-scale ualpha4 according to the operator
                ualpha4 = ualpha4 * operator_coefficient_dict[self.operator]
                ...
                if df['dipole_units'] in dipole_units_dict:
                    # convert transition moment [\mu_B] into dipole moment [1/GeV]
                    ualpha4 = ualpha4 * dipole_units_dict[df['dipole_units']]
                else:
                    raise ValueError(f"Units of experimental data {df['dipole_units']} not defined.")              
                    
                if df['units'] in unit_dict:
                    # fix units to HEP units (GeV)
                    m4 = m4 * unit_dict[df['units']]
                    m4 = m4/1000
                    #m4 = m4/1000 # set units to GeV

#                     if df['id'] in exp_dict:
                    # order data points
                    order = np.argsort(m4)
                    _m4 = m4[order]
                    #_m4 = m4
