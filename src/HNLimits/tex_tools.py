...
def add_plot(doc, case):
    ...
    citations = rf"Constraints on {case.latexflavor} as a function of the HNL mass $m_N$. Limits shown: "
    names = sorted(case.limits.plot_label)
    for name in names:
        inspires_key = list(case.limits.reference[case.limits.plot_label == name])[0].replace(" ", "")
        citations += rf"{name}~\cite{{{inspires_key}}}, ".replace("\\\\", "")
...
def create_latex_doc(cases, TEX_PATH="tex_files/"):
    ...
    doc.append(pylatex.Command("clearpage"))
