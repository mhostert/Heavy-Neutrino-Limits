...
#operator_dict = {'neutral', 'charged'} # selects the kind of bosonic current operator
...
class Limits:
    def __init__(self, flavor='e', invisible=False):
        ...
        if self.flavor == 'neutral':
            self.latexflavor = fr'$C_\text{{HN}}/\Lambda^2 ~(\rm{{GeV}}^{{-2}})$'
        else:
            self.latexflavor = fr'$C_{{HN{subscript}}}/\Lambda^2 ~(\rm{{GeV}}^{{-2}})$'
        self.invisible = invisible
        _df = load_google_sheet(sheet_name=f'Bosonic_{self.flavor}')
        ...
        self.path_to_plot = os.path.join(os.getcwd(), f'plots/bosonic_currents/neutral/CHN_majorana.pdf')
