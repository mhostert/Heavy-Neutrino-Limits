...
def create_latex_doc(cases, TEX_PATH = 'tex_files/'):
    ...
    doc = pylatex.Document(f'{TEX_PATH}/bosonicNC_plots', documentclass=NoEscape(r'revtex4-1'))

    for case in cases:
        add_plot(doc, case)

    with open(f'{TEX_PATH}/bosonicNC_plots.bib','w', encoding='utf-8') as f:
        added_ids = []
        for case in cases:
    ...
    doc.append(pylatex.Command('bibliography', arguments=NoEscape(r'bosonicNC_plots')))
