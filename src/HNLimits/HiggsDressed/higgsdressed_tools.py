...
class Limits:
    def __init__(self, flavor='e', invisible=False, nature='dirac'):
        ...
        self.latexflavor = fr'$C^{subscript}_\text{{LNH}}/\Lambda^2 ~(\rm{{GeV}}^{{-2}})$'
        self.invisible = invisible
        self.nature = nature 
        if self.nature != 'dirac':
            if self.nature != 'majorana':
                raise ValueError(f"Define the right nature of the HNL.")
        _df = load_google_sheet(sheet_name=f'Higgs-dressed_{flavor}')
        ...
        self.path_to_plot = os.path.join(os.getcwd(), f'plots/higgs_dressed/HiggsDressed_{self.flavor}_{self.nature}.pdf')
...
#                    raise ValueError(f"CL of experimental data {df['CL']} not defined.")
                ...
                if self.nature == 'dirac':
                    if df['hnl_type'] == 'Majorana':
                        ...
                        #ualpha4 = ualpha4 * df['D_to_M_factor']
                ...
                if df['units'] in unit_dict:
                    ...
                    # we have to re-scale the mixing^2 ualpha4 to the coefficient of the Higgs-dressed operator C_{LNH}/Lambda^2
                        ualpha4 = (2*sqrt(2)/(246.22)**3)*m4*np.sqrt(ualpha4) #[GeV^-2] with m4 in GeV. See Eq. 4.2 of the draft
