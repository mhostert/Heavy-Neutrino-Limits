...
def create_latex_doc(cases, PATH = 'tex_files/'):
    ...
    doc = pylatex.Document(f'{PATH}/bosonicCC_plots', documentclass=NoEscape(r'revtex4-2'))

    for case in cases:
        add_plot(doc, case)

    with open(f'{PATH}/bosonicCC_plots.bib','w', encoding='utf-8') as f:
        added_ids = []
        for case in cases:
    ...
    doc.append(pylatex.Command('bibliography', arguments=NoEscape(r'bosonicCC_plots')))
