...
def ReT2KCC(M):
    me = 5.11e-4
    if M >= 2*me:
        z =  sqrt((GammaPie(M,1,0,0)+Gammavee(M,1,0,0)+Gammavemmup(M,1,0,0)+Gammavmumu(M,1,0,0))/
...
def ReBEBCCC(M):
    ...
    if M > (mPi+me):
        if M < (mDs-me):
            z = sqrt((GammaPie(M,1,0,0)+Gammavee(M,1,0,0)+Gammavmumu(M,1,0,0)+Gammavemmup(M,1,0,0))/
...
def RmuNUTEVCC(M):
    ...
    mmu = 1.05e-1
    if M >= me + mmu:
        z = sqrt((Gammavmumep(M,0,1,0) + Gammavmumu(M,0,1,0) + GammaPimu(M,0,1,0) + GammaRhomu(M,0,1,0))/
...
def RmuT2KCC(M):
    ...
    mmu = 1.05e-1
    if M >= me + mmu:
        z =  sqrt((GammaPimu(M,0,1,0) + Gammavmumu(M,0,1,0) + Gammavmumep(M,0,1,0) + Gammavee(M,0,1,0))/
...
def RmuLHCCC(M):
    ...
    mmu = 1.05e-1
    if M >= me + mmu:
        z = sqrt((Gammavmumu(M, 0, 1, 0) + Gammavmumep(M, 0, 1, 0) + Gammavee(M, 0, 1, 0))/
                 (GammavmumuCC(M, 0, 1, 0) + Gammavmumep(M, 0, 1, 0)))
    else:
        z = 0
    return z
