...
op_dict = {r'$C_{\text{LNL}\ell}^{\mu e}$': 1, r'$C_{\text{LNL}\ell}^{\tau e}$': 1, r'$C_{\text{LNL}\ell}^{\tau \mu}$': 1}

fermion = 'e'
operator_ee_ln = fr'$\mathcal{{O}}_{{\rm{{{fermion} {fermion}}}}}: (\overline{{{fermion}}} \gamma^{{\mu}} {fermion}) (\overline{{N}} \gamma_{{\mu}} N) \\ \\\mathcal{{O}}_{{\rm{{LN}}}}^{{e}}: (\overline{{L_{{e}}}} \gamma^{{\mu}} L_{{e}}) (\overline{{N}} \gamma_{{\mu}} N)$'
fermion = 'u'
operator_uu = fr'$\mathcal{{O}}_{{\rm{{{fermion} {fermion}}}}}: (\overline{{{fermion}}} \gamma^{{\mu}} {fermion}) (\overline{{N}} \gamma_{{\mu}} N)$'
fermion = 'd'
operator_dd = fr'$\mathcal{{O}}_{{\rm{{{fermion} {fermion}}}}}: (\overline{{{fermion}}} \gamma^{{\mu}} {fermion}) (\overline{{N}} \gamma_{{\mu}} N)$'

#operator_ln = fr'$\mathcal{{O}}_{{\rm{{LN}}}}^{{e}}: (\overline{{L_{{e}}}} \gamma^{{\mu}} L_{{e}}) (\overline{{N}} \gamma_{{\mu}} N)$'
operator_qn = fr'$\mathcal{{O}}_{{\rm{{QN}}}}: (\overline{{Q_{{i}}}} \gamma^{{\mu}} Q_{{i}}) (\overline{{N}} \gamma_{{\mu}} N)$'

operator_lnll_ee = fr'$\mathcal{{O}}_{{\rm{{LNL}}\ell}}^{{ee}}: (\overline{{L_{{e}}}} N)\epsilon (\overline{{L_{{e}}}} e)$'
operator_lnll_ab = fr'$\mathcal{{O}}_{{\rm{{LNL}}\ell}}^{{\alpha\beta}}: (\overline{{L_{{\alpha}}}} N)\epsilon (\overline{{L_{{\alpha}}}} \ell_\beta)$'


o_dic = {'ee_LN': operator_ee_ln, 'uu': operator_uu, 'dd': operator_dd, 
         'QN': operator_qn,
         'LNLlee': operator_lnll_ee, 'LNLlab': operator_lnll_ab}
    ...
    for id, limit in case.limits.iterrows():
        if (id not in skip_ids) & (limit.interp_func is not None):
            ...
            if limit.file_top == limit.file_bottom and limit.m4 is not None and limit.ualpha4 is not None:
                if (limit.operators_involved in op_dict):
                    jlabel= fr'\noindent {limit.operators_involved}'
                    ax.plot(limit.m4, limit.ualpha4, color=contour_color, dashes=dash, zorder=3, lw=LW, label=jlabel)
                else:
                ...
                if (limit.operators_involved in op_dict):
                    ...
                    ax.plot(limit.m4, limit.ualpha4, color=contour_color, dashes=dash, zorder=3, lw=LW)
                #ax.plot(x, limit.interp_func(x), color=contour_color, dashes=dash, zorder=3, lw=LW, label=jlabel)
                #ax.plot(x, limit.interp_func(x), color=contour_color, dashes=dash, zorder=3, lw=LW)
                #jlabel= fr'\noindent {limit.operators_involved}'
                ax.plot(x, limit.interp_func_top(x), color=contour_color, dashes=dash, zorder=3, lw=LW)#, label=jlabel)
